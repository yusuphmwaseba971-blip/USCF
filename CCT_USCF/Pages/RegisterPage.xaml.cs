...
public partial class RegisterPage : ContentPage
{
    ...
    private async void OnCreateAccountClicked(object sender, EventArgs e)
    {
        ...
        // These will be populated from the real database
        // once the location endpoints are connected.
...
private List<LocationItem> _regions = new();
private List<LocationItem> _districts = new();
private List<LocationItem> branches = new();
...
{
    if (RegionPicker.SelectedItem is not LocationItem region)
        return;
    ...
    {
        _districts =
            await _authService.GetDistrictsAsync(region.Id);
...
{
    if (DistrictPicker.SelectedItem is not LocationItem district)
        return;
    ...
    {
        _branches =
            await _authService.GetBranchesAsync(district.Id);
